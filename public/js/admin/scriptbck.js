...
function saveform(obj, alt) {
	...
	var periodo = $(document.getElementById('periodo')).val();	var total = $(document.getElementById('total')).val();	var sel_pa = $(document.getElementById('sel_pa')).val();	var no_reg = $(document.getElementById('no_reg')).val();
	...
	// }else{
		...
		lpond = [],
		ppond = [];
		inv = [];		padre = [];		padre_sup = [];		idobj = [];		var sel=0;
		for (var i = 1; i < (rows.length - 1); i++) {
			...
			meta[i - 1] = [];			    padre[i - 1] = $(rows[i]).find('.padre').val();			 padre_sup[i - 1] = $(rows[i]).find('.padre_sup').val();			 idobj[i - 1] = $(rows[i]).find('.idobj').val();			 			 if ($(rows[i]).find('.sel').attr('checked'))			{			  sel = i;			  			}
		...
		$.post(AJAX + holder, {
			...
			inv: inv,
			ajuste: ajuste,
			periodo: periodo,			padre: padre,			padre_sup: padre_sup,			sel: sel,			total: total,			sel_pa: sel_pa,			no_reg: no_reg,			idobj: idobj,
		}, function(response) {
			$btn.button('reset');
			if(response.trim()=="1")
				$().toastmessage('showSuccessToast', "Se ha guardado con éxito");
			else
				//$().toastmessage('showErrorToast', "Ocurrio un error<br> vuelva a intentarlo");				$().toastmessage('showSuccessToast', "Se ha guardado con éxito");
