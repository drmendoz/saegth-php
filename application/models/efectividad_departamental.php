...
class Efectividad_departamental extends Model {
// **********************
// CLASS DECLARATION
// **********************
...
// **********************
		...
		function select($id){
			$sql =  "SELECT * FROM sonda WHERE id_empresa = $id AND tipo_sonda = 'Efectividad_departamental' order by id desc limit 1";
		...
		function select__(){
			...
			$sql =  "SELECT * FROM sonda WHERE id_empresa = $this->id_empresa AND id=(SELECT max(id) FROM sonda WHERE id_empresa = $this->id_empresa AND tipo_sonda = 'Efectividad_departamental' );";
			$row =  $this->query_($sql,1);
			if($row){
				$this->id = $row['id'];
				$this->id_empresa = $row['id_empresa'];
		...
		function select_($id,$idt){
			...
			$sql =  "SELECT * FROM sonda WHERE id_empresa = $id AND id=$idt AND tipo_sonda = 'Efectividad_departamental'";
			$row =  $this->query_($sql,1);
			if($row){
				$this->id = $row['id'];
				$this->id_empresa = $row['id_empresa'];
		...
		function select_compara($id_e,$sondas = ''){
			...
			$sql =  "SELECT * FROM sonda WHERE id_empresa = $id_e and fecha != '' AND tipo_sonda = 'Efectividad_departamental' ";
			if($sondas)
		...
		function count_evaluados(){
			...
			$sql =  "SELECT count(id) as count FROM sonda WHERE id_empresa = $id AND tipo_sonda = 'Efectividad_departamental'";
			$row =  $this->query_($sql,1);
			return $row['count'];
		}

		function get_fecha_x_id($id_e, $id){
			$sql = "SELECT fecha FROM sonda WHERE id_empresa = ".$id_e." and id = ".$id." AND tipo_sonda = 'Efectividad_departamental'";
			$row =  $this->query_($sql,1);
			$new_fecha = '';
			if($row){
				$fecha = $row['fecha'];
...
// **********************
		...
		function delete(){
			$sql = "DELETE FROM sonda WHERE id = $id; AND tipo_sonda = 'Efectividad_departamental'";
...
// **********************
		...
		function update(){
			...
			$sql = " UPDATE sonda 
					...
					WHERE id = $this->id AND tipo_sonda = 'Efectividad_departamental' ";
			$result = $this->query_($sql);
		...
		function get_Sondas_Empresa($id_e, $flag = '', $arrSondas = ''){
			...
			$sql = "SELECT * FROM sonda WHERE id_empresa = ".$id_e." and fecha != '' AND tipo_sonda = 'Efectividad_departamental' ";
			if ($flag != '') {
				$sql .= "and id <> (select max(id) from sonda where id_empresa = ".$id_e." AND tipo_sonda = 'Efectividad_departamental') ";
			}
