...
function get_sub_id_level_op_($id, $lvl,$aprovado){
  ...
  *
  ...
  p.id_empresa = '.$_SESSION['Empresa']['id'].'
  GROUP BY
  mr.id_evaluado, mr.aprovado, mr.periodo_evaluador ';
  if($aprovado == 1)
  {
    $sql .= 'UNION ALL ';
    $sql .= 'SELECT
  ...
  foreach ($res as $key => $value) {
    ...
    if($isReinicio['id'] != 0)
    {
      if($isReinicio['id_personal'] == $value['id'] && $isReinicio['id'] == $value['periodo_evaluador'])
      {
        $result[$i] = $value;
        $i++;
      }
...
function get_sub_id_level_op__($id, $lvl,$aprovado){
  ...
  *
  ...
  WHERE
  p.id_empresa = '.$_SESSION['Empresa']['id'].'
  GROUP BY mr.id_evaluado, mr.aprovado, mr.periodo_evaluador 
  ) q1
  JOIN (
  SELECT
  `hi`.`id_personal` AS `id`,
...
function count_sub_id_level_op_($id, $lvl,$aprovado){
  $sql = 'SELECT DISTINCT * 
          FROM 
            ( 
              SELECT p.nombre_p AS nombre, id_evaluado AS id, pe.pid_sup AS pid, mr.id_empresa, mr.periodo_evaluador, MAX(aprovado) AS aprovado, mr.nivel as level 
              FROM multifuente_relacion AS mr 
              INNER JOIN personal AS p ON id_evaluado = p.id 
              INNER JOIN personal_empresa AS pe ON pe.id_personal = p.id 
              WHERE p.id_empresa = '.$_SESSION['Empresa']['id'].'
              AND p.Id != '.$id.'
              AND mr.nivel = '.$lvl.'
              GROUP BY mr.id_evaluado, mr.aprovado, mr.periodo_evaluador, mr.nivel 
          ';
  if ($aprovado == 1) {
    $sql .= '    
              UNION ALL 
              ...
              AND p.Id != '.$id.'
              AND mr.nivel = '.$lvl.'
              GROUP BY mr.id_evaluado, mr.id_personal, mr.nivel
          ';
  }
  
  $sql .= '
            ) q1
          WHERE q1.aprovado = '.$aprovado;
  //echo $sql;
  $res = $this->query_($sql);
...
function count_sub_id_level_op__($id, $lvl,$aprovado){
  ...
  FROM
  (
  SELECT DISTINCT p.nombre_p AS nombre, id_evaluado AS id, pe.pid_sup AS pid, mr.id_empresa, mr.periodo_evaluador, MAX(aprovado) AS aprovado 
  FROM multifuente_relacion AS mr
  INNER JOIN personal AS p ON id_evaluado = p.id
  INNER JOIN personal_empresa AS pe ON pe.id_personal = p.id
  WHERE p.id_empresa = '.$_SESSION['Empresa']['id'].'
...
public function multifuente_aprobacion_evaluadores($id, $lvl,$aprovado)
{
  $sql = 'SELECT DISTINCT * 
          FROM 
            ( 
