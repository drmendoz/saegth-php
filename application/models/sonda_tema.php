...
class sonda_tema extends Model{ 
	...
	function select_all_sonda(){

		$sql =  "SELECT id,tema FROM $this->_table WHERE activo = 1 AND tipo = 1";
		return $this->query_($sql);
	}

	function select_all_efectividad(){

		$sql =  "SELECT id,tema FROM $this->_table WHERE activo = 1 AND tipo = 2 ";
		return $this->query_($sql);
	}
	...
	function insert(){
		...
		$sql = "INSERT INTO $this->_table (tipo,tema,descripcion,preguntas ) VALUES ('$this->tipo','$this->tema','$this->descripcion','$this->preguntas' )";
		$result = $this->query_($sql);
	...
	function update(){
		...
		$sql = " UPDATE $this->_table SET  id = '$this->id',tipo = '$this->tipo', tema = '$this->tema',descripcion = '$this->descripcion',preguntas = '$this->preguntas' WHERE id = $this->id ";
	...
	//
	function get_Temas($id_s, $tipo_sonda, $arrTemas = ''){
