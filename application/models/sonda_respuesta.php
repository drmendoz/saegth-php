...
// **********************
	...
	function get_avg_x_tema($ids,$preguntas,$nuevos_criterios){
		...
		if ($nuevos_criterios == 0) {
			...
			$sql = "SELECT AVG(respuesta) as res FROM `sonda_respuestas` WHERE id_suser IN ($ids) AND id_pregunta IN($preguntas) AND respuesta<>0 AND respuesta <> -1";
		}
	...
	function get_avg_x_pregunta($ids,$id,$nuevos_criterios,$min_avg = ''){
		if($nuevos_criterios == 0)
			...
			$sql = "SELECT AVG(respuesta) as res FROM `sonda_respuestas` WHERE id_suser IN ($ids) AND id_pregunta = $id AND respuesta<>0 AND respuesta <> -1 ";
	...
	function get_percent_criterios($sonda, $ids, $preguntas, $min_avg = '')
	{
		$per = array();
		$criterios_rango_barras = $sonda->getCriteriosRangoBarras();

		foreach ($criterios_rango_barras as $key => $value) {
			if ($key == 0)
			{
				// Respuestas 'NO SE'
				$sql = "SELECT count(respuesta) as res 
						FROM `sonda_respuestas` 
						WHERE id_suser IN ($ids) 
						AND id_pregunta IN($preguntas) 
						AND respuesta<>0 
						AND respuesta = -1 ";
				...
				$sql = "SELECT count(respuesta) as res 
						...
						AND id_pregunta IN($preguntas) 
						AND respuesta<>0 
						AND respuesta >= ".$value['desde']." 
						AND respuesta <= ".$value['hasta']."  
						AND respuesta <> -1 ";
		...
		if (sizeof($per) > 0) {
			foreach ($per as $key => $valor) {
				$per[$key] = @round((($valor*100)/$total),2);
			}
		}
	...
	function get_percent_x_pregunta_criterios($sonda, $ids, $preguntas, $min_avg = '')
	{
		$per = array();
		$criterios_rango_barras = $sonda->getCriteriosRangoBarras();
