<?php $meth = new User(); $util = new Util(); //var_dump($final)?>
  ...
  <?php if(isset($final)){ ?>
    <form method="POST" id="oportunidades" action="<?php echo BASEURL.'multifuente/definir_plan' ?>">
      <div class="row form-group">
      <div class="col-md-12">
        <table class="table-bordered" id="oportunidades" >
          <tr>
            ...
            <td class="col-md-<?php echo $col_sep ?>">
              <?php 
                if(isset($final[$rango][$i])){
                  $val = $final[$rango][$i];
                  if($val['val'] < 3.33){
                    echo '<input type="checkbox" name="opt_m[]" value="'.$val['preg'].'" > ';
    ...
    <?php } ?>
      ...
      <div class="row text-center">
        <input type="submit" id="sbmt" class="btn btn-info btn-xs" name="opt" value="Generar plan de acci&oacute;n">
      </div>
    </form>
  <?php } ?>
  ...
  $('#sbmt').click(function(){
    var chk = AreAnyCheckboxesChecked();
    if(!chk){
      var r = confirm('No se ha seleccionado ninguna oportunidad');
      if(r)
        document.getElementById("oportunidades").submit();
      else
    ...
    function AreAnyCheckboxesChecked () {
      var checkboxes =  document.forms.oportunidades.elements
