...
<ul style="list-style-type: none;" id="pregunta_g">
	<?php
	$tema = new Evaluacion_desempenio_pregunta();
	$res = $tema->select_x_tema_($id, 0, 1);
	foreach ($res as $key => $value) {
		$preg = new Evaluacion_desempenio_pregunta($value);
...
<script type="text/javascript">
	...
	function delete_pregunta(id_p,ul_id){
		var holder = 'elim_eval_desemp_pregunta';
		var li = 'li_'+id_p;
		$.post(AJAX + holder, {
			id_p:id_p
		}, function(response) {
	...
	$('#add').on('click', function () {
		...
		$.post(AJAX + 'eval_desemp_pregunta_ajax', {
			tema:$('#id_tema').val(),
	...
	$('#txtTema, #descripcion').on('blur', function () {
		$.post(AJAX + 'eval_desemp_tema_desc_ajax', {
			id:$('#id_tema').val(),
			tema:$('#txtTema').val(),
