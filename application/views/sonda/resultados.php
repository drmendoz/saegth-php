...
?>
<form  action="<?php echo BASEURL.'sonda/resultados/'.$id_s ?>" method="POST">
	...
	<div class="collapse" id="collapseTemas" style="width: 400px;">
		<div class="well">
			<select multiple name="temas[]" class="form-control">
				<option style="display:none">Seleccione una opción</option>
				<?php
				$x->get_Temas($id_s,$tipo_sonda,$arrTemas);
				?>
			</select>
		</div>
	</div>
	...
	<table class="table table-bordered table-hover" style="margin-top:50px">
			...
			foreach ($temas as $key => $value) {
				if (isset($arrTemas) && is_array($arrTemas)) {
					if (in_array($key, $arrTemas)) {
						...
						//
						echo "<tr>";
							echo "<td class='col-md-7'><h4><strong><a href='".BASEURL."sonda/resultados_pregunta/".$id_s."/".$tema_id."'>".$tema_nombre."</a></strong></h4>";
		...
		<tr>
			...
			<td class="text-center">
				<div class="btn-group">
					<a target="_blank" href="<?php echo BASEURL ?>sonda/top/<?php echo $id_s; ?>/DESC" class="btn btn-default">Mejor puntaje</a>
					<a target="_blank" href="<?php echo BASEURL ?>sonda/top/<?php echo $id_s; ?>/ASC" class="btn btn-default">Peor puntaje</a>
					<a target="_blank" href="<?php echo BASEURL ?>sonda/comentarios/<?php echo $id_s; ?>" class="btn btn-default">Comentarios Foda</a>
					<a href="<?php echo BASEURL ?>pdf/sonda/<?php echo $id_s."/".$filtros."/".$array_para_enviar_via_url ?>" class="btn btn-default">Generar reporte</a>
