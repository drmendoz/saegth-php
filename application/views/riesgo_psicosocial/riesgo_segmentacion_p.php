...
<div class="container-fluid my-own-class">
	<table id="tableall" class="table table-bordered table-hover consulta">
		<thead>
			<tr><th colspan="<?php echo $colspan; ?>"><h5><strong><a href="<?php echo BASEURL ?>riesgo_psicosocial/riesgo_segmentacion"><?php echo ucfirst($x->getTema()) ?>.-</a></strong> 	<?php echo $x->getDescripcion() ?></h5></th></tr>
			<tr>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<?php
		...
		?>
			<tr>
				...
				foreach ($arrCompara as $cont => $arrDatos) {
					...
					$graf[$key.'|'.$id_pregunta][$arrDatos['nombre']] = ($porcentaje != 0) ? number_format($porcentaje, 2) : $porcentaje;
					$arrFechas[serialize($arrDatos)] = $arrDatos['nombre'];
					$arrPromedios[serialize($arrDatos)][] = number_format($porcentaje, 2);
		...
		?>
			<tr>
				...
				foreach ($arrCompara as $cont => $arrDatos) {
					$ids = $z->get_id_x_segmentacion($_SESSION['Empresa']['id'], $riesgos, $arrDatos['campo'], $arrDatos['id']);
					$porcentaje = $w->get_percent($ids,$preguntas_general);
					$style = $w->get_color($porcentaje);
					array_push($arrPromedios[serialize($arrDatos)], number_format($porcentaje, 2));
					$graf[$riesgos.'|'.'Promedio General'][$arrDatos['nombre']] = ($porcentaje != 0) ? number_format($porcentaje, 2) : $porcentaje;
				?>
				<td style="text-align:right;background-color: <?php echo $style; ?> ;min-width:80px;"><h4><?php echo $porcentaje; ?>%</h4></td>
