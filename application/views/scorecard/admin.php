    ...
    <table class="table-bordered scorecard" id="table" style="width: 100%; line-height: 12px;">
      ...
      <tbody>
        <tr ng-repeat="item in items track by $index">
          <td style="text-align: left !important"><a href="{{baseurl}}scorecard/confirmacion/{{item.id}}">{{item.nombre}}</a></td>
          <td>{{item.cargo}}</td>
          <td compile="item.usuario"></td>
          <td compile="item.jefe"></td>
          <td compile="item.bloqueo"></td>
          <td compile="item.revision"></td>
          <td compile="item.evaluacion"></td>
          <td compile="item.revision_jefe"></td>
          <td compile="item.evaluacion_jefe"></td>
          <td>{{item.resultado_scorer_p}}</td>
          <td>{{item.resultado_scorer}}</td>
          <td>{{item.resultado_compass}}</td>
          <td>{{item.resultado_ponderado}}</td>
          <td compile="item.form"></td>
        </tr>
      </tbody>
    </table>
  </div>
...
<script type="text/javascript">
  ...
  app.controller('formCtrl', function($scope,$log,$window,$timeout, $http, $sce) {
    ...
    var url=$window.AJAX;
    url = url + "scorecard_admin/";

    $scope.getData = function(lim){
      var result = $http.get(url+lim,{cache: true,params:{"lim": lim}});
