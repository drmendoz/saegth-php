...
<script type="text/javascript">
	$(document).ready(function(){
		...
		if(TemaSonda != ''){
			...
			$('#temas option').remove();
        	var holder = 'temas_x_efectividad_ajax';
        	var config_ = {
				cache: true,
				headers: {
		...
		$('#li_cres_efect').addClass('activate');
		var cols = [];
...
$sonda_individual = new Efectividad_departamental();
...
?>
<h4>DIAGNÓSTICO DE EFECTIVIDAD ORGANIZACIONAL.- COMPARAR ENCUESTAS</h4>
<form action="<?php echo BASEURL.'efectividad_departamental/comparar_resultados' ?>" method="POST">
	<h4>
	...
	<table id="tableall" class="table table-bordered table-hover consulta" style="margin-top:50px">
		...
		<tbody>
			...
			foreach ($temas as $key => $value) {
				if (isset($temas[$key]) && is_array($temas[$key]) && sizeof(($temas[$key]) > 0)) {
					...
					if (in_array($key, $arrTemas)) {
						...
						echo "<tr>";
						echo "<td style='min-width:500px; max-width:500px; white-space:normal;'><h4><strong><a href='".BASEURL."efectividad_departamental/compara_resultados_pregunta/".$tema_id."'>".$tema_nombre."</a></strong></h4>";
						echo "<h5>".$x->getDescripcion()."</h5></td>";

						foreach ($sondas_compara as $cont => $id_sonda) {
							$sonda_individual->select_($_SESSION['Empresa']['id'],$id_sonda);
							$nuevos_criterios = $sonda_individual->getNuevosCriterios();
							$criterios_escala = $sonda_individual->getCriteriosEscala();
							$criterios_barras_colores = $sonda_individual->getCriteriosBarrasColores();
							$criterios_rango_barras = $sonda_individual->getCriteriosRangoBarras();
							...
							if ($nuevos_criterios == 0) {
								...
								$porcentajes = $xls_prep[$key]['porcentajes'] = $w->get_percent_criterios($sonda_individual, $ids,$preguntas);
							}
						...
						foreach ($sondas_compara as $cont => $id_sonda) {
							...
							$temas[$key] = @array_values($temas[$key]);
  							$temas[$key] = @array_values($temas[$key]);
				...
				<?php
					...
					foreach ($sondas_compara as $cont => $id_sonda) {
						...
						if ($nuevos_criterios == 0) {
							...
							$porcentajes_generales = $w->get_percent_criterios($sonda_individual, $ids,$preguntas_general);
						}
			...
			<tr>
				...
				<td class="text-center">
					<div class="btn-group">
						<a href="<?php echo BASEURL ?>efectividad_departamental/top/<?php echo $id_sonda; ?>/DESC" class="btn btn-default">Mejor puntaje</a>
						<a href="<?php echo BASEURL ?>efectividad_departamental/top/<?php echo $id_sonda; ?>/ASC" class="btn btn-default">Peor puntaje</a>
						<a href="<?php echo BASEURL ?>pdf/efectividad_compara/<?php echo $id_sonda; ?>/'<?php echo $filtros."/".$array_para_enviar_via_url ?>" class="btn btn-default">Generar reporte</a>
						<a href="<?php echo BASEURL ?>pdf/efectividad_xls_compara/<?php echo $id_sonda."/".$array_para_enviar_via_url ?>" class="btn btn-default">Descargar en excel</a>
					</div>
				</td>
				<?php
				}
