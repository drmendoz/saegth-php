...
$util = new Util();
$tipo_sonda = 'Efectividad_departamental';

echo '<h4>DIAGNÓSTICO DE EFECTIVIDAD ORGANIZACIONAL.- GRUPOS CRITICOS</h4>';
echo "<br>";
?>
<form action="<?php echo BASEURL.'efectividad_departamental/grupos_criticos' ?>" method="POST">
...
if (is_array($arrSondas)) {
	$rendimiento->select_tipo($_SESSION['Empresa']['id'], $tipo_sonda);
	...
	$rendimiento->promedios_bajos($_SESSION['Empresa']['id'],$tipo_sonda);
	//
				...
				foreach ($arrValores as $id_tema => $arrTema) {
					...
					echo "<tr>";
						...
						foreach ($arrSondas as $key => $id_sonda) {
							...
							$nuevos_criterios = $rendimiento->getNuevosCriterios();
							$criterios_escala = $rendimiento->getCriteriosEscala();
							$criterios_barras_colores = $rendimiento->getCriteriosBarrasColores();
							$criterios_rango_barras = $rendimiento->getCriteriosRangoBarras();
							$temas = $rendimiento->getTemas();
											...
											foreach ($arrPreguntas as $key => $id_preg) {
												...
												if (in_array($id_preg, $arrTemasPreguntas)) {
													echo '<table border="0" width="100%">';
														echo '<tr>';
															if($id_sonda == $last_id){
																$promedio = $w->get_avg_x_pregunta($ids, $id_preg, $nuevos_criterios, 3);

																if($nuevos_criterios == 0)
																	$porcentajes = $w->get_percent_x_pregunta($ids,$id_preg, 3);
																else
																	$porcentajes = $w->get_percent_x_pregunta_criterios($rendimiento,$ids,$id_preg, 3);
															}else{
																$promedio = $w->get_avg_x_pregunta($ids, $id_preg, $nuevos_criterios, 3);
																...
																if($nuevos_criterios == 0)
																	...
																	$porcentajes = $w->get_percent_x_pregunta_criterios($rendimiento,$ids,$id_preg);
															}
