...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#tblRspEnc input[type=text]').keyup(function (){
			...
			if ($('#nuevos_criterios').val() == 0) {
				// Solo para las sondas anteriores, es decir, las que no tienen los nuevos parametros de escalas
...
?>
<form id="target" class="form" method="POST" action="<?php echo BASEURL ?>efectividad_departamental/test_temp/<?php echo $id_e ?>/<?php echo $id_t ?>">
	...
	<h4 class="form-group">EFECTIVIDAD - Esta encuesta es absolutamente anónima y confidencial o sea, No interesa saber quien contestó de una u otra forma. lo que <strong>SI</strong> interesa es que sea <u>muy sincero</u> en sus respuestas.</h4>
	<?php
	...
	<table class="table table-bordered table-hover" id="tblRspEnc">
		...
		foreach ($temas as $tkey => $tval) {
			...
			foreach ($preguntas as $s_key => $s_value) {
				...
				if($_SESSION['USER-AD']['user_rol'] == 10 || $_SESSION['USER-AD']['user_rol'] == 12){
					$y->getOptions($sonda);
				}else{
	...
	<?php
	$sonda_f = new Efectividad_departamentalController('Efectividad_departamental','efectividad_departamental','test_temp_foda',0,true,true);
	$sonda_f->test_temp_foda($id_e,$id_t);
	$sonda_f->ar_destruct();
	unset($sonda_f);
	?>
	<input type="hidden" id="id_test" name="id_test" value="<?php echo $id_t ?>">
	<input type="hidden" id="nuevos_criterios" name="nuevos_criterios" value="<?php echo $nuevos_criterios ?>">
