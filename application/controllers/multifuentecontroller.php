...
class MultifuenteController extends Controller {
	...
	function asignar_test(){
		...
		if(isset($_POST['button'])){
			foreach ($_POST['id_per'] as $a => $c) {
				...
				<p>Le solicitamos también que conteste este cuestionario como màximo hasta el ".$_POST['f_max'].". A fin de facilitarle la comprensión del uso y sobretodo de los beneficios de esta herramienta le invitamos a ver una corta presentación a través de  <a href = 'https://youtu.be/wSog56gtU5U?si=hCOgzhOtCt7O2VNM'>Video</a> <br></p>


				<br>Desde ya le agradecemos su colaboración. <br><br>";

				$msg .=	"<p>Vínculo.- <a href ='".BASEURL."'>".BASEURL."</a></p>";
				$msg .=	"<table><tr><td><b>Usuario<b></td><td>".$user_pid->user_name."</td></tr><tr><td><b>Contraseña<b></td><td>".$user_pid->token."</td></tr></table>";

				Util::sendMail($email,$subject,$msg);

				$mpe = new Multifuente_periodos_evaluador();
				$isReinicio = $mpe->last_periodoEvaluador($c, $_SESSION['Empresa']['id']);
				$periodo_evaluador = $isReinicio['id'];

				$this->Multifuente->query('INSERT INTO `multifuente_evaluado`(`id_personal`, `cod_evaluado`, `cod_test`, `fecha`, `fecha_max`, `id_empresa`, `periodo_evaluador`) VALUES ('.$c.',"'.$cod_evaluado.'","'.$_POST['test'].'","'.$fecha.'","'.$_POST['f_max'].'",'.$_SESSION['Empresa']['id'].','.$periodo_evaluador.')');
				//echo mysqli_error($this->link);
				$subject="Alto Desempeño - Evaluación multifuentes de ". $nombre ."";
				$evaluadores=$this->Multifuente->query_('SELECT id_personal,relacion FROM multifuente_relacion WHERE id_evaluado='.$c.' AND aprovado=3 AND periodo_evaluador='.$periodo_evaluador);
				foreach ($evaluadores as $key => $b) {
					...
					$this->Multifuente->query('INSERT INTO `multifuente_evaluadores`(`id_personal`,`id_evaluado`, `cod_evaluado`, `cod_test`, `fecha`, `fecha_max`, `id_empresa`, `relacion`, `periodo_evaluador`) VALUES ('.$b.','.$c.',"'.$cod_evaluado.'","'.$_POST['test'].'","'.$fecha.'","'.$_POST['f_max'].'",'.$_SESSION['Empresa']['id'].','.$rel.','.$periodo_evaluador.')');
					...
					<br><p>Le solicitamos también que conteste este cuestionario como màximo hasta el ".$_POST['f_max'].". A fin de facilitarle la comprensión del uso y sobretodo de los beneficios de esta herramienta le invitamos a ver una corta presentación a través de  <a href = 'https://youtu.be/wSog56gtU5U?si=hCOgzhOtCt7O2VNM'>Video</a> <br><br></p>
			...
			$mp->id_empresa = $_SESSION['Empresa']['id'];
			$Reinicio = $mp->tiene_reinicio();

			if(!$Reinicio)
			{
				$sql = 'SELECT DISTINCT p.id, p.nombre,p.cargo,p.area,p.foto, ifnull(me.cod_test,"No ha sido asignado") as codigo,ifnull(me.fecha,"-") as fecha_e FROM listado_personal_op as p 
				...
				$sql = 'SELECT DISTINCT p.id, p.nombre, p.cargo, p.area, p.foto 
						FROM listado_personal_op AS p 
						JOIN multifuente_relacion AS mr 
						JOIN multifuente_periodos_evaluador AS mpe ON 	mpe.id_personal = mr.id_evaluado AND 
																		mpe.id = mr.periodo_evaluador AND 
																		mpe.id_empresa = mr.id_empresa AND 
																		mpe.activo = "A" 
						JOIN multifuente_periodos AS mp ON mp.id = mpe.periodo AND 
															mp.id_empresa = mr.id_empresa AND 
															mp.activo = "A" 
						JOIN personal_test AS pt ON p.id = mr.id_evaluado 
						WHERE mr.id_empresa = '.$_SESSION['Empresa']['id'].' 
						AND pt.`compass_360` =1
						AND pt.id_personal = mr.id_evaluado 
						AND mr.aprovado = 3 ';
				//echo $sql;

				$evaluadoDistinct = $this->Multifuente->query_($sql);
				$evaluado = array();
				$idx = 0;

				foreach ($evaluadoDistinct as $key => $value) {
					...
					$sql = 'SELECT * 
							...
							WHERE me.id_personal = '.$value['id'].' 
							AND me.periodo_evaluador = '.$periodo_evaluador.' 
							';
					//echo "$idx - $sql<br>";

					$idx++;

					$multifuente_evaluado = $this->Multifuente->query_($sql,1);
					
					/*if(count($multifuente_evaluado) > 1)
					{
						$evaluado[$idx]['id'] = $value['id'];
						$evaluado[$idx]['nombre'] = $value['nombre'];
						$evaluado[$idx]['cargo'] = $value['cargo'];
						$evaluado[$idx]['area'] = $value['area'];
						$evaluado[$idx]['foto'] = $value['foto'];
						$evaluado[$idx]['codigo'] = $multifuente_evaluado['cod_test'];
						$evaluado[$idx]['fecha_e'] = $multifuente_evaluado['fecha'];
					}
					else
					{
						$evaluado[$idx]['id'] = $value['id'];
						...
						$evaluado[$idx]['area'] = $value['area'];
						$evaluado[$idx]['foto'] = $value['foto'];
						$evaluado[$idx]['codigo'] = "No ha sido asignado";
						$evaluado[$idx]['fecha_e'] = "-";
					}*/

					// Solo los que no se han asignado test
					if(count($multifuente_evaluado) > 0)
					{
						$evaluado[$idx]['id'] = $value['id'];
						$evaluado[$idx]['nombre'] = $value['nombre'];
						$evaluado[$idx]['cargo'] = $value['cargo'];
	...
	function asignar_test_all(){
		$this->haySession();
		$this->set('backlink','multifuente/asignar_all');
		...
		if(!$Reinicio)
		{
			$sql = 'SELECT DISTINCT p.id, p.nombre,p.cargo,p.area,p.foto, ifnull(me.cod_test,"No ha sido asignado") as codigo,ifnull(me.fecha,"-") as fecha_e FROM listado_personal_op as p 
				...
				JOIN personal_test as pt 
				ON p.id=ea.id 
				JOIN multifuente_evaluado as me
				ON p.id=me.id_personal
				WHERE ea.id_empresa='.$_SESSION['Empresa']['id'].' 
				AND pt.`compass_360`=1 
				AND pt.id_personal=ea.id
			...
			foreach ($evaluadoDistinct as $key => $value) {
				...
				// Solo los que se han asignado test
				if(count($multifuente_evaluado) > 0)
				{
					$evaluado[$idx]['id'] = $value['id'];
					$evaluado[$idx]['nombre'] = $value['nombre'];
	...
	*/
	...
	function resultados($id=null,$test=null,$eval=null){
		...
		$res = $this->get_Eval($id,$test,$eval,$_SESSION['multifuente']['periodo_evaluador']);
		if(isset($res)){
			$temas=array();
	...
	function plan($id,$eval=null){
		...
		// $id=$_SESSION['Personal']['id'];
		$res = $this->get_Eval($id,null,null,$_SESSION['multifuente']['periodo_evaluador']);
		// $res = reset($res);
		...
		if(isset($_POST['button'])){
			foreach ($_POST['id'] as $a => $id) {
				...
				foreach ($evaluadores as $key => $value) {
					...
					$message = self::get_MsgEvaluadores($nombre,$nombre_evaluado,$fecha,$cod_evaluado,$usr,$fecha_max,$value['id_personal']);
				...
				foreach ($evaluado as $key => $value) { 
					...
					$fecha_max = $value['fecha_max'];
					$cod_evaluado = $value['cod_evaluado'];
					$message = self::get_MsgEvaluado($nombre,$fecha,$cod_evaluado,$usr,$fecha_max,$value['id_personal']);
					Util::sendMail($email,$subject,$message);				
				}
			}
	...
	function estatus_evaluado($_id){
			...
			foreach ($_POST['id'] as $a => $id) {
				...
				foreach ($res as $key => $value) { 
					if($res){
						...
						$message .= "<p>Puede ver esta y otras evaluaciones pendientes siguiendo el vínculo en el menú lateral bajo Compass 360 > Cuestionarios Pendientes.</p><p>Si por alguna razón debe interrumpir la respuesta al cuestionario, por favor asegúrese de hacer click en el Botón “Grabar y Continuar luego” que está al final del cuestionario.  Usted podrá entrar y salir del cuestionario tantas veces lo requiera utilizando estas mismas credenciales de acceso (más abajo indicadas) sin embargo, le recomendamos enfáticamente que trate de contestar de una sola vez el cuestionario completo.</p><p>Le solicitamos también que conteste este cuestionario como máximo hasta el ".$value['fecha_max'].". A fin de facilitarle la comprensión del uso y sobretodo de los beneficios de esta herramienta le invitamos a ver una corta presentación a través de  <a href = 'https://youtu.be/wSog56gtU5U?si=hCOgzhOtCt7O2VNM'>Video</a> </p><p>Desde ya le agradecemos su colaboración. </p>";
						$message .=	"<p>Vínculo.- <a href ='".BASEURL."'>".BASEURL."</a></p>";
						$message .=	"<table><tr><td><b>Usuario<b></td><td>".$user_pid->user_name."</td></tr><tr><td><b>Contraseña<b></td><td>".$user_pid->token."</td></tr></table>";
		...
		$mpe = new Multifuente_periodos_evaluador();
		$row = $mpe->last_periodoEvaluador($_id, $_SESSION['Empresa']['id']);
		$sql = 'SELECT `id_personal`,`fecha`,`cod_evaluado`,`relacion`,`resuelto` FROM `multifuente_evaluadores` WHERE `id_evaluado`='.$_id.' and periodo_evaluador = '.$row['id'].' ORDER BY `fecha` DESC';
		
		$evaluadores=$this->Multifuente->query_($sql);
		...
		$evaluado=$this->Multifuente->query_('SELECT `id_personal`,`fecha`,`cod_evaluado`,`relacion`,`resuelto` FROM `multifuente_evaluado` WHERE `id_personal`='.$_id.' and periodo_evaluador = '.$row['id'].' ORDER BY `fecha` DESC');	
		$this->set('evaluado',$evaluado); 
	}
	...
	function reiniciar()
	{
		...
		if (isset($_POST['reinicio_individual'])) {

			if (!$mp->tiene_reinicio()) {
				$mp->anio = date('Y');
				$mp->activo = 'A';
				$mp->insert();
			}
			...
			if(isset($_POST['id_per'])){
				foreach ($_POST['id_per'] as $key => $id_personal) {
					$mpe->periodo = $mp->getId();
					$mpe->id_personal = $id_personal;
					$mpe->id_empresa = $_SESSION['Empresa']['id'];
					$mpe->reinicio_indivudual = 1;
					$mpe->activo = 'A';
					$mpe->inactiveAllPersonal($mpe->id_personal,$mpe->id_empresa);
					$mpe->insert();

					// Envio de correos a: Administrador - Jefe - Jefe del Jefe
					$nombre_personal = $this->Multifuente->get_pname($id_personal);
					$results = Personal_empresa::get_superior($id_personal);
					$jefe = $results[0]['pid_sup'];
					$email_jefe = $this->Multifuente->get_emailById($jefe);
					$results = Personal_empresa::get_superior($jefe);
					$jefe_jefe = $results[0]['pid_sup'];
					$email_jefejefe = $this->Multifuente->get_emailById($jefe_jefe);

					$msg =  "<p>Estimado,</p>";
					$msg .= "Se ha reiniciado la evaluación de Compass 360 para ".$nombre_personal;

					$subj = "Reinicio de evaluación de Compass 360";
					Util::sendMail($email_jefe,$subj,$msg);
					Util::sendMail($email_jefejefe,$subj,$msg);
					Util::sendMail($emp->email,$subj,$msg);
				}

			}
		}

		if (isset($_POST['reinicio_masivo'])) {
			...
			foreach ($evaluado as $a => $b) {
				...
				$id_personal = $b['id'];
				$mpe->inactiveAllPersonal($id_personal,$_SESSION['Empresa']['id']);
				$mpe->setPeriodo($mp->getId());
				$mpe->setId_personal($id_personal);
				$mpe->setId_empresa($_SESSION['Empresa']['id']);
				$mpe->setActivo('A');
				$mpe->insert();
	...
	*/
	...
	function get_Eval($id,$test=null,$eval=null,$periodo_evaluador=null){		
		...
		if(isset($test) && isset($eval)){
			...
			$res = $this->Multifuente->query_('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$id.' AND `cod_evaluado`="'.$_SESSION['evaluado']['id'].'" AND `cod_test`="'.$_SESSION['evaluado']['test'].'" AND periodo_evaluador = "'.$periodoEvaluador.'"',1);
			if($res){
				$otras = $this->Multifuente->query_('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$id.' AND periodo_evaluador='.$periodoEvaluador.' ORDER by fecha DESC');
				if((sizeof($otras))<2){
					$_SESSION['evaluado']['otras']=null;
				}else{
					$_SESSION['evaluado']['otras']=$otras;
				...
				$res = $this->Multifuente->query_('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$id.' AND periodo_evaluador='.$periodoEvaluador.' ORDER by fecha DESC');
				if($res){
					if((sizeof($res))<2){
						$_SESSION['evaluado']['otras']=null;
						$res = $res[0];
	...
	function get_MsgEvaluadores($nombre,$nombre_evaluado,$fecha,$cod_evaluado,$usr,$fecha_max,$id_personal){
		$user_pid = new User();
		$user_pid->select($id_personal);
		
		$message = "<p>Estimado(a) ".$nombre.",</p><p>Este es un recordatorio automático para que complete la evaluación de ".$nombre_evaluado ." la cual le fue enviada el ".$fecha.".</p>";
		$message .= "<p>Puede ver esta y otras evaluaciones pendientes siguiendo el vínculo en el menú lateral bajo Compass 360 >  Cuestionarios Pendientes.</p><p>Si por alguna razón debe interrumpir la respuesta al cuestionario, por favor asegúrese de hacer click en el Botón “Grabar y Continuar luego” que está al final del cuestionario.  Usted podrá entrar y salir del cuestionario tantas veces lo requiera utilizando estas mismas credenciales de acceso (más abajo indicadas) sin embargo, le recomendamos enfáticamente que trate de contestar de una sola vez el cuestionario completo.</p><p>Le solicitamos también que conteste este cuestionario como máximo hasta el ".$fecha_max.". A fin de facilitarle la comprensión del uso y sobretodo de los beneficios de esta herramienta le invitamos a ver una corta presentación a través de  <a href = 'https://youtu.be/wSog56gtU5U?si=hCOgzhOtCt7O2VNM'>Video</a> </p><p>Desde ya le agradecemos su colaboración. </p>";
		$message .=	"<p>Vínculo.- <a href ='".BASEURL."'>".BASEURL."</a></p>";
		$message .=	"<table><tr><td><b>Usuario<b></td><td>".$user_pid->user_name."</td></tr><tr><td><b>Contraseña<b></td><td>".$user_pid->token."</td></tr></table>";
		return $message;
	}
	...
	function get_MsgEvaluado($nombre,$fecha,$cod_evaluado,$usr,$fecha_max,$id_personal){
		$user_pid = new User();
		$user_pid->select($id_personal);
		
		$message = "<p>Estimado(a) ".$nombre.",</p><p>Este es un recordatorio automático para que complete su evaluación; la cual le fue enviada el ".$fecha.".</p>";
		$message .= "<p>Puede ver esta y otras evaluaciones pendientes siguiendo el vínculo en el menú lateral bajo Compass 360 >  Cuestionarios Pendientes.</p><p>Si por alguna razón debe interrumpir la respuesta al cuestionario, por favor asegúrese de hacer click en el Botón “Grabar y Continuar luego” que está al final del cuestionario.  Usted podrá entrar y salir del cuestionario tantas veces lo requiera utilizando estas mismas credenciales de acceso (más abajo indicadas) sin embargo, le recomendamos enfáticamente que trate de contestar de una sola vez el cuestionario completo.</p><p>Le solicitamos también que conteste este cuestionario como máximo hasta el ".$fecha_max.". A fin de facilitarle la comprensión del uso y sobretodo de los beneficios de esta herramienta le invitamos a ver una corta presentación a través de  <a href = 'https://youtu.be/wSog56gtU5U?si=hCOgzhOtCt7O2VNM'>Video</a> </p><p>Desde ya le agradecemos su colaboración. </p>";
		$message .=	"<p>Vínculo.- <a href ='".BASEURL."'>".BASEURL."</a></p>";
		$message .=	"<table><tr><td><b>Usuario<b></td><td>".$user_pid->user_name."</td></tr><tr><td><b>Contraseña<b></td><td>".$user_pid->token."</td></tr></table>";
		return $message;
	...
	function estatus_proceso(){
		...
		$mep = new Multifuente_estatus_proceso();
		$mep->obtenerDatos();
		
		$this->set('listado',$mep->getlistado());
		$this->set('filtro',$mep->getfiltro());
		$this->set('fecha_inicio',$mep->getfecha_inicio());
		$this->set('fecha_tope',$mep->getfecha_tope());
		$this->set('fecha_actual',$mep->getfecha_actual());
		$this->set('dias_transcurridos',$mep->getdias_transcurridos());
		$this->set('dias_restantes',$mep->getdias_restantes());
		$this->set('dias_adicionales',$mep->getdias_adicionales());
		$this->set('cuestionarios_asignados',$mep->getcuestionarios_asignados());
		$this->set('cuestionarios_respondidos',$mep->getcuestionarios_respondidos());
		$this->set('cuestionarios_no_respondidos',$mep->getcuestionarios_no_respondidos());
		$this->set('ratio_cumplimiento_fecha',$mep->getratio_cumplimiento_fecha());
		$this->set('ratio_cumplimiento_dia1',$mep->getratio_cumplimiento_dia1());
		$this->set('ratio_cumplimiento_dia2',$mep->getratio_cumplimiento_dia2());
		$this->set('ratio_cumplimiento_proyectado1',$mep->getratio_cumplimiento_proyectado1());
		$this->set('ratio_cumplimiento_proyectado2',$mep->getratio_cumplimiento_proyectado2());
		$this->set('ratio_cumplimiento_proyectado_actual1',$mep->getratio_cumplimiento_proyectado_actual1());
	}
}
