...
class SondaController extends Controller {
	...
	function __construct($model, $controller, $action, $type = 0,$full=false,$render=false) {
		...
		$this->link = $this->Sonda->getDBHandle();
		...
		if(array_filter($_POST)){
			...
			if(isset($_POST['submit'])){
				...
				*/
				...
				$subject = 'Encuesta en Línea de Clima Organizacional Sonda';
				...
				//$link = BASEURL.'sonda/encuesta/'.Util::nav_encode($last_user);
				$link = BASEURL;
				

 				$msj = '<p>Estimado/a,</p>';

				$msj .= '<p>Use el Vínculo, Usuario y Contraseña, proporcionados abajo, para abrir la plantilla de ingreso de respuestas a la Encuesta de Diagnóstico de Clima Organizacional "SONDA" aplicadas en Línea.</p>';
				...
				*/
				...
				$subject = 'Encuesta en Papel de Clima Organizacional Sonda';
				...
				$msg .= '<p>Use el Vínculo, Usuario y Contraseña, proporcionados abajo, para abrir la plantilla de ingreso de respuestas a la Encuesta de Clima Organizacional "SONDA" aplicadas en Papel.</p>';
				...
				if($custom_email == 1){
					...
					$result_lp = $this->Sonda->query_($sql);

	 				$subject = 'Encuesta Individual de Clima Organizacional Sonda';
				...
				if(isset($_POST['guardar'])){
					...
					$this->_template = new Template('Sonda','definir');
					$this->definir();
	...
	function encuesta($id)
	{
		...
		if ($id) {
			$dispatch = new AjaxController('Ajax','ajax','link_encuesta',0);
			$dispatch->link_encuesta($id);
		}
	}
	...
	function comparar_resultados(){
		...
		if(isset($_POST['filtro'])){
			...
			$areas=$edad=$antiguedad=$localidad=$edad=$tcont=$educacion=$sexo=$temaSonda=$temas=$filtros="";
    ...
    */
	...
	function grupos_criticos(){
		...
		$sql =  "SELECT MAX(ID) as id FROM sonda WHERE id_empresa = ".$_SESSION['Empresa']['id']." AND tipo_sonda = 'Sonda' ";
		...
		if(isset($_POST['button'])){
			foreach ($_POST['id'] as $a => $id) {
				...
				if ($a <= 50) {
					...
					$pass = $result['token'];
					$nombre = $this->Sonda->get_pname($id);
					$email = $this->Sonda->get_emailById($id);
	...
	function get_area(&$cod){
		...
		$res = $this->Sonda->query('SELECT `nombre` FROM `empresa_area` WHERE `id`="'.$cod.'"',1);
		...
		return $this->Sonda->htmlprnt($res);
	...
	function get_localidad($cod){
		...
		$res = $this->Sonda->query('SELECT `nombre` FROM `empresa_local` WHERE `id`="'.$cod.'"',1);
	...
	function get_cargo($cod){
		...
		$res = $this->Sonda->query('SELECT `nombre` FROM `empresa_cargo` WHERE `id`="'.$cod.'"',1);
	...
	function get_norg($cod){
		...
		$res = $this->Sonda->query('SELECT `nombre` FROM `empresa_norg` WHERE `id`="'.$cod.'"',1);
	...
	function get_tcont($cod){
		...
		$res = $this->Sonda->query('SELECT `nombre` FROM `empresa_tcont` WHERE `id`="'.$cod.'"',1);
	...
	function getFechas($id_e){
		...
		$sql = 'SELECT id, fecha FROM sonda WHERE id_empresa = '.$id_e.' and fecha != "" AND tipo_sonda = "Sonda" order by id desc';

		$res = $this->Sonda->query_($sql);
	...
	function get_sondas($id_s){
		...
		$sql = 'SELECT fecha FROM sonda WHERE id_empresa = '.$_SESSION['USER-AD']['id_empresa'].' and id = '.$id_s.' AND tipo_sonda = "Sonda"';
	...
	function get_temas($cod){
		...
		$res = $this->Sonda->query('SELECT `tema` FROM `sonda_temas` WHERE `id`="'.$cod.'"',1);
		...
		return ucfirst($this->Sonda->htmlprnt($res));
