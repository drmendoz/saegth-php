...
class PdfController extends Controller {
	...
	function sonda($id_s, $filtros="", $arrTema=""){
		...
		foreach ($temas as $key => $value) {
			...
			if (isset($arrTema) && is_array($arrTema)) {
				if (in_array($key, $arrTema)) {
					...
					$promedio = $w->get_avg_x_tema($ids,$preguntas,$nuevos_criterios);

					if ($nuevos_criterios == 0)
						$porcentajes = $w->get_percent($ids,$preguntas);
					else
						$porcentajes = $w->get_percent_criterios($sonda, $ids,$preguntas);
						...
						if (is_array($criterios_barras_colores)) {
							foreach ($criterios_barras_colores as $key => $color) {
								...
								if ($key > 0) {
									...
									$pdf->SetFillColor(250,250,250);
									$pdf->Cell((65-$ancho),6.25,$porcentajes[$key].'%','R',2,'R',false);
								}
							}

							$ancho = ($porcentajes[0] * 60) / 100;
							$ancho = ($ancho == 0) ? 0.5 : $ancho;

							list($r, $g, $b) = sscanf($criterios_barras_colores[0], "#%02x%02x%02x");
							$pdf->SetFillColor($r, $g, $b);
							...
							$pdf->SetFillColor(250,250,250);
							$pdf->Cell((65-$ancho),6.25,$porcentajes[0].'%','R',2,'R',false);
						}
					}
		...
		if ($nuevos_criterios == 0 ) {
			...
			if (is_array($criterios_barras_colores)) {
				foreach ($criterios_barras_colores as $key => $color) {
					$ancho = ($porcentajes_generales[$key] * 60) / 100;
					$ancho = ($ancho == 0) ? 0.5 : $ancho;
					...
					if ($key > 0) {
						...
						$pdf->SetFillColor(250,250,250);
						$pdf->Cell((65-$ancho),6.25,$porcentajes_generales[$key].'%','R',2,'R',false);
					}
				}

				$ancho = ($porcentajes_generales[0] * 60) / 100;
				$ancho = ($ancho == 0) ? 0.5 : $ancho;
				...
				$pdf->SetFillColor(250,250,250);
				$pdf->Cell((65-$ancho),6.25,$porcentajes_generales[0].'%','R',2,'R',false);
			}
		}		
				...
				if (in_array($key, $arrTema)) {
					...
					foreach ($preguntas as $p_key => $p_value) {
						...
						if($nuevos_criterios == 0)
							...
							$porcentajes = $w->get_percent_x_pregunta_criterios($sonda,$ids,$id_p);
	...
	function sonda_xls($id_s, $arrTema=""){
		...
		foreach ($temas as $key => $value) {
			...
			if (isset($arrTema) && is_array($arrTema)) {
				...
				$promedio = $w->get_avg_x_tema($ids,$preguntas);echo "$promedio".php_eol;
	...
	function efectividad_departamental($id_s, $filtros="", $arrTema=""){
		...
		$this->dir=$this->location.'efectividad_departamental'.DS.$nemp;
		...
		foreach ($temas as $key => $value) {
			...
			if (isset($arrTema) && is_array($arrTema)) {
				if (in_array($key, $arrTema)) {
					...
					$pdf->Cell(10,31.25,$indice,1,0,'C');
					...
					$pdf->Line(20,31.25+($yy),200,31.25+($yy));
					...
					$pdf->Cell(10,31.25,number_format($promedio,2),1,0,'C',true);
		...
		$pdf->Cell(115,31.25,"Promedio general:",1,0,'L',false); 

		$pdf->Cell(10,31.25,number_format($p_gen,2),1,0,'L',true);

		$pdf->SetFillColor(250,250,250);

		$pdf->SetFont('Times','',9);

		$pdf->Line(20,31.25+($pdf->GetY()),200,31.25+($pdf->GetY()));
	...
	function estatus_proceso_xls($filtro)
	{
		Util::sessionStart();
		...
		$style = array('font' => array('bold' => true, 'size' => 16,),'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT,),);
		$style2 = array('font' => array('bold' => true, 'size' => 14,),'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT,),);
		$style3 = array('font' => array('bold' => false, 'size' => 12,),'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT,),);
		...
		$ews->setTitle('Estatus');

		$ews->setCellValue('a1', 'ESTADO DEL PROCESO COMPASS AL '.date('d-M-Y'));

		$ews->setCellValue('b1', '');

		$ews->getStyle('a1')->applyFromArray($style);

		$ews->mergeCells('b1:c1');

		$ews->getColumnDimension('a')->setAutoSize(true);

		$ews->setCellValue('a3', 'EVALUADORES');

		$ews->getStyle('a3')->applyFromArray($style2);

		$ews->setCellValue('b3', 'CARGO');

		$ews->getStyle('b3')->applyFromArray($style2);

		$ews->getColumnDimension('b')->setAutoSize(true);

		$ews->setCellValue('c3', '% CUMPLIMIENTO');
		...
		if (count($listado) > 0){
			foreach ($listado as $cod_evaluador => $arrayEvaluados) {
				...
				$ews->setCellValue('a'.$indice, $this->Pdf->htmlprnt_win($result["nombre"]));
				$ews->getStyle('a'.$indice)->applyFromArray($style3);

				$ews->setCellValue('b'.$indice, $this->Pdf->htmlprnt_win($result["cargo"]));
				$ews->getStyle('b'.$indice)->applyFromArray($style3);

				$ews->setCellValue('c'.$indice, $explode[1].'%');
				$ews->getStyle('c'.$indice)->applyFromArray($style3);
				...
				if (count($arrayEvaluados) > 0) {
					foreach ($arrayEvaluados as $key => $value) {
						...
						$ews->setCellValue('a'.$indice, '        '.$this->Pdf->htmlprnt_win($result2["nombre"]));
						$ews->getStyle('a'.$indice)->applyFromArray($style3);

						$ews->setCellValue('c'.$indice, '        '.$value["cumplimiento"].'%');
						$ews->getStyle('c'.$indice)->applyFromArray($style3);
		...
		//
		$nombre = 'Reporte de estado del proceso Compass 360';
		...
		// Redirect output to a client’s web browser (Excel5)
		  header('Content-Type: application/vnd.ms-excel');
		  header('Content-Disposition: attachment;filename="estadoprocesocompass360.xlsx"');
		  header('Cache-Control: max-age=0');
	...
	function estatus_proceso_pdf($filtro)
	{
		...
		$mep = new Multifuente_estatus_proceso();
		$mep->setfiltro($filtro);

		$mep->obtenerDatos();

		$listado = $mep->getlistado();
		$fecha_inicio = $mep->getfecha_inicio();
		$fecha_tope = $mep->getfecha_tope();
		$dias_transcurridos = $mep->getdias_transcurridos();
		$dias_restantes = $mep->getdias_restantes();
		$cuestionarios_asignados = $mep->getcuestionarios_asignados();
		$cuestionarios_respondidos = $mep->getcuestionarios_respondidos();
		$cuestionarios_no_respondidos = $mep->getcuestionarios_no_respondidos();
		$ratio_cumplimiento_fecha = $mep->getratio_cumplimiento_fecha();
		$ratio_cumplimiento_dia1 = $mep->getratio_cumplimiento_dia1();
		$ratio_cumplimiento_dia2 = $mep->getratio_cumplimiento_dia2();
		$ratio_cumplimiento_proyectado1 = $mep->getratio_cumplimiento_proyectado1();
		$ratio_cumplimiento_proyectado2 = $mep->getratio_cumplimiento_proyectado2();
		$empresaCab = $_SESSION['Empresa']['nombre'];
		...
		$pdf->Cell(100,10,date('d/m/Y'),0,0,'L');
		...
		$pdf->Cell(170,10,"ESTADO DEL PROCESO COMPASS360 AL ".date('d-m-Y'),0,0,'C');

		$pdf->Ln(15);

		$pdf->Cell(170,10,"Detalle por: ".$filtro,0,0,'C');

		$pdf->SetFont('Times','',12);

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Fecha inicio:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$fecha_inicio,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Fecha tope:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$fecha_tope,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Días transcurridos:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$dias_transcurridos,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Días restantes:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$dias_restantes,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Cuestionarios totales asignados:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$cuestionarios_asignados,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Cuestionarios respondidos:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$cuestionarios_respondidos,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Cuestionarios NO respondidos:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$cuestionarios_no_respondidos,0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Ratio de cumplimiento a la fecha:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(90,5,$ratio_cumplimiento_fecha.'%',0,0,'L');

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Ratio de cumplimiento diario:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(45,5,$ratio_cumplimiento_dia1,0,0,'L');

		$pdf->Cell(45,5,$ratio_cumplimiento_dia2.'%',0,0,'L');		

		$pdf->Ln(15);

		$pdf->Cell(91,5,"Ratio de cumplimiento proyectado a fecha tope:",0,0,'L');

		$pdf->SetFont('Times','',12);

		$pdf->Cell(45,5,$ratio_cumplimiento_proyectado1,0,0,'L');

		$pdf->Cell(45,5,$ratio_cumplimiento_proyectado2.'%',0,0,'L');
		...
		$pdf->Cell(75,5,'Evaluador',1,0,'L');

		$pdf->Cell(30,5,'Respondido',1,0,'C');

		$pdf->Cell(75,5,'Persona a evaluar',1,1,'L');

		if (count($listado)) {
			foreach ($listado as $key => $value) {
				$pdf->SetFont('Times','',10);

				$pdf->Cell(75,5,$this->Pdf->htmlprnt_win($value['evaluador']),1,0,'L');

				if($this->Pdf->htmlprnt_win($value['resuelto']) == 1)
					$pdf->Cell(30,5,$this->Pdf->htmlprnt_win('Si'),1,0,'C');
				else
					$pdf->Cell(30,5,$this->Pdf->htmlprnt_win('No'),1,0,'C');

				$pdf->Cell(75,5,$this->Pdf->htmlprnt_win($value['persona_evaluar']),1,1,'L');
			}
		}

		//
		//$nombre = $this->Pdf->get_pname($id);
		$nombre = 'Reporte de estado de proceso Compass360°';
		//$nombre = str_replace(" ", "", $nombre);
		self::makeDir($this->dir);
		$pdf->Output($this->dir.DS.$nombre.'.pdf', "F" );
		$pdf->Output($nombre.'.pdf','D');
