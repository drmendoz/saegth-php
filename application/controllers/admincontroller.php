			...
			if (isset($_POST['nuevaEmpresa'])){
				...
				if(isset($_POST['eval'])){
					foreach ($_POST['eval'] as $key => $eval) {
						...
						switch ($eval) {
							case 1:
								$empresa->setCompass_360(1); //'`compass_360`';
								break;
								case 2:
								$empresa->setScorer(1); //'`scorer`';
								break;
								case 3:
								$empresa->setMatriz(1); //'`matriz`';
								break;
								case 4:
								$empresa->setValoracion(1); //'`valoracion`';
								break;
								case 5:
								$empresa->setClima_laboral(1); //'`sonda`';
								break;
								case 6:
								$empresa->setRetencion(1); //'`riesgo de retencion`';
								break;
								case 7:
								$empresa->setCobertura(1); //'`cobertura`';
								break;
								case 8:
								$empresa->setPsicosocial(1); //'`riesgo psicosocial`';
								break;
							}
						}
					}
					...
					if(!$empresa->getError()){
						...
						if(Util::sendMail($empresa->email,$subject,$mensaje)){
							...
							$this->set('custom_success',"Se ha agregado correctamente la empresa.");
						}else{
							$this->set('custom_warning',"Se ha agregado correctamente la empresa, pero occurrió un error al procesar el correo.");
						}
					}
					$_POST=null;
				}
