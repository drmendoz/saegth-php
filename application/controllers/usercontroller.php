...
class UserController extends Controller {
	...
	// Obtenemos Periodo-Evaluador actual del usuario logueado para poder consultar y crear transacciones a partir de ese id obtenido en todas las tablas multifuente
		$mp = new Multifuente_periodos();
	    $mp->setId_empresa($_SESSION['USER-AD']['id_empresa']);
	    $mp->last_periodo();	    
		$_SESSION['multifuente']['periodo'] = $mp->id;

		$mpe = new Multifuente_periodos_evaluador();
		$result = $mpe->last_periodoEvaluador($_SESSION['USER-AD']['id_personal'],$_SESSION['USER-AD']['id_empresa']);
		$_SESSION['multifuente']['periodo_evaluador'] = $result['id'];
	}
...
*/
	...
	function multifuente($cod_evaluado=null) {
		...
		$mpe = new multifuente_periodos_evaluador();
		$isReinicio = $mpe->last_periodoEvaluador(self::get_id($cod_evaluado), $_SESSION['Empresa']['id']);
		$periodo_evaluador = $isReinicio['id'];

		if(self::get_id($cod_evaluado) == $_SESSION['USER-AD']['id_personal']){
			$result = $this->User->query('SELECT `id`,`resuelto`,`cod_test`,`id_personal` FROM `multifuente_evaluado` WHERE `cod_evaluado` ="'. $cod_evaluado .'" AND periodo_evaluador = '.$periodo_evaluador,1);
			$table = 'multifuente_evaluado';
			$rel = "Auto";
		}else{
			$result = $this->User->query('SELECT `id`,`resuelto`,`cod_test`,`id_personal`,`relacion` FROM `multifuente_evaluadores` WHERE `id_personal` ="'. $_SESSION['USER-AD']['id_personal'] .'" And `cod_evaluado` ="'. $cod_evaluado .'" AND periodo_evaluador ='.$periodo_evaluador,1);
		...
		$test = $this->User->query('SELECT DISTINCT `pregunta`,`cod_pregunta`,`cod_tema` FROM `multifuente_test` WHERE `cod_test` ="'. $result['cod_test'] .'" AND `cod_pregunta` NOT IN(SELECT `cod_pregunta` FROM `multifuente_respuestas` WHERE `cod_evaluado`="'.$cod_evaluado.'" AND `id_personal`='.$_SESSION['USER-AD']['id_personal'].')');
		$this->set('test',$test);
		...
		if(isset($_POST['terminar'])){
			...
			for($j=0;$j<=$_POST['cant'];$j++){
				if(isset($_POST[$j."_multi"])){
					...
					$sql = "INSERT INTO multifuente_respuestas(cod_test,cod_tema,cod_pregunta,respuesta,cod_evaluado,fortalezas,debilidades,comentarios,rango,id_personal,periodo_evaluador) VALUES ('".$cod_test."','".$cod_tema."','".$cod_pregunta."','".$respuesta."','".$cod_evaluado."','".$fortalezas."','".$debilidades."','".$comentarios."', '".$rel."',".$user.",'".$periodo_evaluador."')";
					$this->User->query($sql);
				}
			}
			if($_POST['terminar'] == 'Terminar'){
				if(!mysqli_error($this->link)){
					$sql = 'UPDATE '.$table.' SET `resuelto`=1 WHERE `id_personal`='.$_SESSION['USER-AD']['id_personal'].' AND cod_evaluado="'.$cod_evaluado.'" AND periodo_evaluador='.$periodo_evaluador;
					$this->User->query($sql);
	...
	function asignar_relacion(){
		...
		$sel = $mr->select_all_nivel($id,1,$_SESSION['multifuente']['periodo_evaluador']);
		if(!$sel){
			$evaluadores = array();		
		...
		if(isset($_POST['id_per'])){
			...
			if($pid){
				...
				$nombre = $this->User->get_name($pid);
				
				$user_pid = new User();
				$user_pid->select($pid);
				$name_pid = $this->User->query('SELECT * FROM `personal` WHERE `id`=' . $pid . '',1);
				$name_pid = reset($name_pid);

				//$msg="<p>Estimado/a, ".$nombre."</p>";
				$msg =	"<p>Estimado/a, " . $name_pid["nombre_p"] . "</p>";
				$msg .="<p>".$_SESSION['Personal']['nombre']." ha seleccionado sus evaluadores para la evaluación Compass 360. Es necesario que confirme esta selección antes de proceder, puede descartar a y/o seleccionar nuevos evaluadores.</p><p>En el menú lateral encontrara la opción para confirmar bajo Compass 360 > Aprobación de Evaluadores > subalternos directos</p>";
			...
			foreach ($_POST['id_per'] as $a => $b) {
				...
				$args = array(
					...
					"nivel" => $nivel,
					"aprovado" => $aprob,
					"periodo_evaluador" => $_SESSION['multifuente']['periodo_evaluador']
					);
	...
	// $id_evaluado = $_SESSION['evaluado']['id'];
		...
		//By JPazmino
		$result = $mpe->last_periodoEvaluador($id_evaluado,$_SESSION['USER-AD']['id_empresa']);
		$periodo_evaluador = $result['id'];

		$nivel_max = $mr->get_min_nivel($id_evaluado,$periodo_evaluador);
		$all_sel = $mr->select_all($id_evaluado,$periodo_evaluador);
		$sel = $mr->select_all_nivel($id_evaluado,$nivel_max,$periodo_evaluador);
		$this->set('subtitle','Evaluadores seleccionados');
		$sup=$par=$sub=array();
		$tipo_ingreso = "";
		...
		if(isset($_POST['button_r'])){
			...
			$name = $this->User->query('SELECT * FROM `personal` WHERE `id`=' . $id_evaluado . '',1);
			$name = reset($name);
			...
			if($pid){
				if($nivel_max==1){
					...
					$msg =	"<p>Estimado/a, " . $name_pid["nombre_p"] . "</p>";
					$msg .=	"<p>".$_SESSION['Personal']['nombre']." ha revisado y aprobado los evaluadores de ".$name["nombre_p"]." para la evaluación Compass 360. Es necesario que confirme esta selección antes de proceder, puede descartar a y/o seleccionar nuevos evaluadores.</p><p>En el menú lateral encontrara la opción para confirmar bajo Compass 360.- Aprobación de Evaluadores (Subalternos Directos y/o Indirectos)</p>";
					$msg .=	"<p>Vínculo.- <a href ='".BASEURL."'>".BASEURL."</a></p>";
					$msg .=	"<table><tr><td><b>Usuario<b></td><td>".$user_pid->user_name."</td></tr><tr><td><b>Contraseña<b></td><td>".$user_pid->token."</td></tr></table>";
					
					$email = $this->User->get_emailById($pid);
					$subj = "Selección de evaluadores para ".$name["nombre_p"];
					Util::sendMail($email,$subj,$msg);
				}
				elseif($nivel_max==2)
				{
			...
			if ($tipo_ingreso == 'E') {
				...
				$sql = 'SELECT * FROM multifuente_evaluado WHERE id_empresa = '.$_SESSION['USER-AD']['id_empresa'].' AND id_personal = '.$id_evaluado.' AND periodo_evaluador = '.$periodo_evaluador.' ORDER BY id DESC LIMIT 1';
				...
				$last_cod = $me->getLastCodEvaluado($id_evaluado,$test,$periodo_evaluador);
			...
			foreach ($_POST['id_per'] as $a => $b) {
				...
				if($mr->select_id_eval_id_personal($id_evaluado,$val[0],$periodo_evaluador)){
					...
					$args = array(
						...
						"nivel" => $nivel,
						"aprovado" => $aprob,
						"tipo_ingreso" => ($tipo_ingreso == 'E') ? 'E' : '',
						"periodo_evaluador" => $periodo_evaluador
						);
			...
			if ($tipo_ingreso == 'E') {
				if ($nivel == '3' && $aprob == '3') {
					...
					$sql .= 'WHERE id_evaluado='.$id_evaluado.' ';
					$sql .= 'AND aprovado=3 ';
					$sql .= 'AND id_personal in('.substr($new_evaluadores, 0, -1).') ';
					$sql .= 'AND periodo_evaluador = '.$periodo_evaluador.' ';
					...
					foreach ($evaluadores as $key => $b) {
						...
						$this->User->query('INSERT INTO `multifuente_evaluadores`(`id_personal`,`id_evaluado`, `cod_evaluado`, `cod_test`, `fecha`, `fecha_max`, `id_empresa`, `relacion`, `periodo_evaluador`) VALUES ('.$b.','.$id_evaluado.',"'.$cod_evaluado.'","'.$test.'","'.$fecha.'","'.$f_max.'",'.$_SESSION['Empresa']['id'].','.$rel.','.$periodo_evaluador.')');
						...
						<br><p>Le solicitamos también que conteste este cuestionario como màximo hasta el ".$f_max.". A fin de facilitarle la comprensión del uso y sobretodo de los beneficios de esta herramienta le invitamos a ver una corta presentación a través de  <a href = 'https://youtu.be/wSog56gtU5U?si=hCOgzhOtCt7O2VNM'>Video</a> <br><br></p>
	...
	function tests($dir,$atts=null){
		...
		switch ($dir) {
			...
			if($ent->clima_laboral){
				...
				unset($_SESSION['temas']);
				unset($_SESSION['arrTemas']);
			...
			if($ent->clima_laboral){
				...
				unset($_SESSION['args']);
				unset($_SESSION['sondas']);
				unset($_SESSION['filtros']);
				unset($_SESSION['temaSonda']);
				unset($_SESSION['temas']);
				unset($_SESSION['arrSondas']);
				unset($_SESSION['arrTemas']);
            ...
            */
			...
			$ent = Empresa::withID($_SESSION['Empresa']['id']);
			if($ent->clima_laboral){
				$dispatch = new Efectividad_departamentalController('Efectividad_departamental','efectividad_departamental','historicos');
				$dispatch->historicos();
				unset($_SESSION['args']);
				unset($_SESSION['filtros']);
				unset($_SESSION['temas']);
				...
				header("Location: " . BASEURL.'efectividad_departamental/historicos');
			}else
			echo $nope;
			break;
			...
			$ent = Empresa::withID($_SESSION['Empresa']['id']);
			if($ent->clima_laboral){
				$dispatch = new Efectividad_departamentalController('Efectividad_departamental','efectividad_departamental','comparar_resultados');
				$dispatch->comparar_resultados();
				unset($_SESSION['args']);
				unset($_SESSION['sondas']);
				unset($_SESSION['filtros']);
				...
				header("Location: " . BASEURL.'efectividad_departamental/comparar_resultados');
			}else
			echo $nope;
			break;
