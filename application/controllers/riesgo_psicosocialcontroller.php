		...
		if(isset($_POST['guardar'])){
			...
			*/
			...
			while ($count_user != 1) {
				...
				$pass = Util::passHasher($nombre,rand(1000,2000));
			...
			while ($count_user != 1) {
				$user = Util::passHasher($nombre,rand(2000,3000));
				$pass = Util::passHasher($nombre,rand(3000,4000));
				$count_user = $user_class->verificarUsername($user, $pass);
				if($count_user < 1)
					break;
 					...
 					if (is_array($arrMails)) {
 						...
 						foreach ($arrMails as $key => $correo) {
							...
							$count_user = 0;
			 				...
			 				while ($count_user != 1) {
			 					$user = Util::passHasher($nombre,rand(4000,5000));
								$pass = Util::passHasher($nombre,rand(5000,6000));
			 					$count_user = $user_class->verificarUsername($user, $pass);
			 					if($count_user < 1)
			 						break;
