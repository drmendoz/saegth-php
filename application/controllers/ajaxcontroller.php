...
//test
	...
	function eval_desemp_tema_ajax(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_Desempenio_tema();
			$obj->setTema($_REQUEST['tema']);
			$obj->setTipo($_REQUEST['tipo']);
			$obj->insert();
			if($obj->getError())
	...
	function eval_desemp_competencias_ajax(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_Desempenio_Competencia();
			$obj->setCompetencias($_REQUEST['competencias']);
			$obj->insert();
			if($obj->getError())
				echo "error,";
			else
	...
	function eval_desemp_tema_desc_ajax(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_Desempenio_tema();
			$obj->select($_REQUEST['id']);
			...
			if($obj->getError())
				...
				echo "1,<a href='".BASEURL."evaluacion_desempenio/pregunta/".$obj->getId()."' class='text-upper'>".$obj->getTema()."</a><br>,";
		}
	}
	...
	function eval_desemp_pregunta_ajax(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_desempenio_pregunta();
			$obj->setId_tema($_REQUEST['tema']);
			$obj->setId_empresa($_REQUEST['empresa']);
			$obj->setPregunta($_REQUEST['pregunta']);
			$obj->setInverso(0);
	...
	function notification(){
		...
		*/
		...
		$response = array(
				...
				'indirecto' => $pe->count_sub_id_level_op_($_SESSION['USER-AD']['id_personal'],2,2), 
				'pendiente' => $me->count_unresolved__($_SESSION['USER-AD']['id_personal']), 
	...
	function elim_eval_desemp_pregunta(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_desempenio_pregunta();
			$obj->setId($_REQUEST['id_p']);
	...
	function eval_desemp_tema_elimina(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_Desempenio_tema();
			$obj->setId($_REQUEST['id_t']);
			...
			if($obj->getError()){
				...
				$obj_P = new Evaluacion_desempenio_pregunta();
				$obj_P->setId_tema($_REQUEST['id_t']);
				$obj_P->setActivo(0);
				$obj_P->activa_inactiva_preguntas();
				if($obj_P->getError())
	...
	function eval_desemp_competencias_elimina(){
		$this->haySession();
		if($_REQUEST){
			$obj = new Evaluacion_Desempenio_Competencia();
			$obj->setId($_REQUEST['id_t']);
	...
	function temas_x_efectividad_ajax(){
		$this->haySession();
	}
	
	function link_encuesta($id){

		// Esto sirve para Sonda, Efectividad departamental, Riesgo psicosocial

		$id = mysqli_real_escape_string($this->link,$id);
		$id = Util::nav_decode($id);

		$userArr = $this->Ajax->query(' select * from users where id = \''. $id .'\' ');
		...
		$this->Ajax->query_($q,1);
		
		
		//$header_info=$this->getHeaderInfo($_SESSION['USER-AD']["user_rol"]);
		$res = $this->Ajax->query('SELECT navbar,link FROM navbar WHERE user_rol='.$_SESSION['USER-AD']["user_rol"].'',1);
		$header_info = $res['Navbar'];
	...
	function evaluadores_por_evaluado_ajax()
	{
		$this->haySession();
		if($_REQUEST){
			$mep = new Multifuente_estatus_proceso();
			$array = $mep->obtenerEvaluadoresPorEvaluado($_REQUEST['id_evaluado']);
			$final = array();

			foreach ($array as $key_evaluador => $arrayEvaluador) {
				foreach ($arrayEvaluador as $key => $value) {
					$tmp = array(
						'id_personal' => trim($value['id_personal']),
						'nombre' => $mep->htmlprnt_win(trim($value['nombre'])),
						'relacion' => $mep->htmlprnt_win(trim($value['relacion'])),
						'resuelto' => trim($value['resuelto']), 
						'cumplimiento' => trim($value['cumplimiento'])
					);
	...
	function notificacion_compass_ajax()
	{
		$this->haySession();
		if($_REQUEST){
			$mep = new Multifuente_estatus_proceso();
			$id_evaluador = $_REQUEST["codigo_evaluador"];
			$mep->notificacionEvaluadores($id_evaluador);
