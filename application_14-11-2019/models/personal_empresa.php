...
function count_sub_id_level_op_($id, $lvl,$aprovado){
  ...
  {
    $sql .= 'UNION ALL ';
    $sql .= 'SELECT DISTINCT p.nombre_p AS nombre, id_evaluado AS id, pe.pid_sup AS pid, mr.id_empresa, mr.periodo_evaluador, MAX(aprovado) AS aprovado 
    FROM multifuente_relacion AS mr
    INNER JOIN personal AS p ON id_evaluado = p.id
    INNER JOIN personal_empresa AS pe ON pe.id_personal = p.id
    WHERE p.id_empresa = '.$_SESSION['Empresa']['id'].'
    AND mr.tipo_ingreso = "E" 
    GROUP BY mr.id_evaluado, mr.id_personal ';
  }
  $sql .='
  ) q1
  JOIN (
