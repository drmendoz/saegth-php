	...
	<div class="tab-content" style="margin-top:15px">
		...
		<div role="tabpanel" class="tab-pane fade col-md-12" id="tabs-2">
			...
			<div class="col-md-12 no-padding">
				...
				$ceval = array_filter($compass->get_Eval($_SESSION['USER-AD']['id_personal'],null,null,$_SESSION['multifuente']['periodo_evaluador']));print_r($ceval);

				if(null == ($ceval))
				{
					$rel = new Multifuente_relacion();
					//$eval_rel = $rel->select_all($_SESSION['USER-AD']['id_personal'],$_SESSION['multifuente']['periodo_evaluador']);
					$eval_rel = $rel->select_all__($_SESSION['USER-AD']['id_personal'],$_SESSION['multifuente']['periodo_evaluador']);
					if ($eval_rel)
					{
						echo "<p>Usted ya selecciono a sus evaluadores, se esta esperando una confirmación.</p>";
					}
					else
					{
						//echo '<p>Antes de poder asignarle una evaluación debe confirmar la relación de sus evaluadores. <a class="bg-info" href="'.BASEURL.'user/asignar_relacion">Asignar relación</a></p>';
						echo '<p><a class="bg-info" href="'.BASEURL.'user/asignar_relacion">ESCOGA Y/O REVISE A SUS EVALUADORES</a></p>';

						// Verificar esta parte, despues de revisar
						echo "<br><br>";
						$res = $compass->query('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$_SESSION['USER-AD']['id_personal'].' ORDER by fecha DESC');
						
						
						echo "<table>";
					    	echo '<thead align="center">';
					        	echo '<tr>';
					          		echo '<th align="center">Reportes Compass 360 previos</th>';
					          	echo '<th align="center">Fecha</th>';
					        	echo '</tr>';
					      	echo '</thead>';
					      	echo '<tbody align="left">';
					      	foreach ($res as $a => $b) {
					      		$c = $b['Multifuente_evaluado'];
					       		echo '<tr>';
									echo '<td class="col-md-4"><p><a href="'.BASEURL.'multifuente/resultados/'.$_SESSION['USER-AD']['id_personal'].'/'.$c['cod_test'].'/'.$c['cod_evaluado'].'">'.$meth->get_test($c['cod_test']).'</a></p></td>';
									echo '<td class="col-md-4"><p>'.date("F j, Y", strtotime($c['fecha'])).'</p></td>';
					      		echo '</tr>';
					      	}
					    	echo '</tbody>';
					    echo '</table>';
