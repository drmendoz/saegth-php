		...
		<table class="table table-bordered table-striped table-hover table-heading tablesorter" id="datatable">
			...
			<tbody>
				...
				<?php foreach ($evaluados as $a => $b) { 
					$porcentaje = $meth->get_progressByEval($b['id_evaluado']); 
					$resuelto = ($b['resuelto']) ? "Sí" : "No" ;
					?>
					<tr>
						<td><a href="<?php echo BASEURL.'multifuente/estatus_evaluado/'.$b['id_evaluado'] ?>"><?php echo  $meth->htmlImage_($b['foto'],'img-rounded sm-img').$meth->htmlprnt($b['nombre']); ?></a></td>
						<td><?php echo $meth->htmlprnt($b['cargo']) ?></td>
						<td><?php echo $meth->htmlprnt($b['area']) ?></td>
						<td><?php echo $resuelto ?></td>
						<td><?php printf("%.2f%%", $porcentaje); ?><div hidden><input type="text" name="id[]" value="<?php echo $b['id_evaluado']; ?>"></div></td>
					</tr>
					<?php } ?> 
					<!-- End: list_row -->
				</tbody>
