<?php
	if ($existe) {
		$meth = new User();
		$ids=array();
		$ids[]=$_SESSION['USER-AD']['id_personal'];

?>
	<style type="text/css">
	  .empty-row{padding: 1px !important}
	</style>
	<form id='form' action="<?php echo BASEURL ?>multifuente/edit_eval_curso" method="POST">
	  <div class="row">
	          ...
	          <table class="table table-bordered table-striped table-hover table-heading table-datatable" id="<?php echo $key_[1] ?>">
	            ...
	            <tbody>
	              ...
	              <?php foreach ($value as $d => $e) { 
	                $b = new Personal();
	                $c = new Personal_empresa();
	                $c->select($e['id_personal']);
	                $b->select($e['id_personal']); 
	                $ids[]=$e['id_personal'];
	                ?>
	                <tr>
	                  <td><input type="checkbox" name="id_per[]" value="<?php echo $b->id.','.$key_[1]; ?>" checked disabled></td>
	                  ...
	                  <td><?php echo $meth->htmlprnt($meth->get_cargo($c->id_cargo)); ?><div ><input class="relacion" type="hidden" name="rel[]" value="<?php echo $key_[1] ?>"></div></td>
	                </tr>
	                <?php } ?> 
	                <?php } ?>
	                <!-- End: list_row -->
    ...
    <script type="text/javascript">
	    ...
	    $('#asignar_relacion').click(function(event){
	      ...
	      console.log(event);
	      var r = confirm("¿Has seleccionado a todos y tus evaluares?. Si continúa la selección que haya hecho, completa o no, se tendrá como definitiva y será enviada a su supervisor directo para su aprobación/ modificación.");
	      if (r == true) {
	        $('#form').submit();
	      } else {
...
<?php
		...
		echo '<h3 class="bg-warning text-center" style="padding:10px;">No hay evaluación en curso</h3>';
	}
?>