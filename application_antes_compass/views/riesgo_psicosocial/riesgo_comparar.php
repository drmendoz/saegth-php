...
<form action="<?php echo BASEURL.'riesgo_psicosocial/riesgo_comparar'?>" method="POST">
	...
	<div class="collapse" id="collapseEncuentas" style="width: 200px;">
		<div class="well">
			<select multiple name="riesgos[]" class="form-control">
				...
				<?php
				$riesgo->get_Riesgos_Empresa($_SESSION['Empresa']['id'], '', $arrRiesgos);
				?>
			</select>
		</div>
	</div>
	...
	<table class="table table-bordered table-hover" style="margin-top:50px">
		<thead>
			<tr>
				...
				?>
				<th class="text-center"><h4><b><?php echo $fecha_riesgo; ?></b></h4></th>
				<?php
				}
				?>
		...
		?>
			<tr>
				<td style="font-size: 14px; min-width: 550px;">
					<h5>
						<strong>
							<a href="<?php echo BASEURL ?>riesgo_psicosocial/riesgo_comparar_p/<?php echo $id_t ?>">
				...
				<?php
				foreach ($riesgos_compara as $cont => $id_riesgo) {
					$fecha_riesgo = $riesgo->get_fecha_x_id($_SESSION['Empresa']['id'], $id_riesgo);
					$ids = $z->get_id_x_riesgo($args, $id_riesgo);
					$porcentajes = number_format($w->get_percent($ids,$preguntas), 2);
					$graf[$id_t.'|'.$tema_nombre][$fecha_riesgo.'|'.$yy] = ($porcentajes != 0) ? number_format($porcentajes, 2) : $porcentajes;

					if($porcentajes != 0){
						$promedio_general[$id_riesgo][$ii] = $porcentajes;
						$ii++;
					}

					$arrFechas[$id_riesgo] = $fecha_riesgo;
					$arrPromedios[$id_riesgo][] = number_format($porcentajes, 2);
		...
		?>
			<tr>
				...
				<?php
					...
					foreach ($riesgos_compara as $cont => $id_riesgo) {
						...
						$graf[$id_riesgo.'|'.'Promedio General'][$fecha_riesgo.'|'.$yy] = ($porcentajes != 0) ? number_format($porcentajes, 2) : $porcentajes;
						array_push($arrPromedios[$id_riesgo], number_format($porcentajes, 2));
			...
			<tr>
				...
				<td class="text-center">
					<div class="btn-group">
						<a href="<?php echo BASEURL ?>riesgo_psicosocial/top/1/<?php echo $id_riesgo ?>/RC" class="btn btn-default">Puntaje más alto</a>
						<a href="<?php echo BASEURL ?>riesgo_psicosocial/top/0/<?php echo $id_riesgo ?>/RC" class="btn btn-default">Puntaje más bajo</a>
						<a href="<?php echo BASEURL.'pdf/rp/'.$_SESSION['Empresa']['id'].'/'.$id_riesgo.'/'.$args_url.'/'.$filtros_url ?>" class="btn btn-default">Generar reporte</a>
						<a href="<?php echo BASEURL.'pdf/rp_xls/'.$_SESSION['Empresa']['id'].'/'.$id_riesgo.'/'.$args_url.'/'.$filtros_url ?>" class="btn btn-default">Descargar en excel</a>
					</div>
				</td>
				<?php
				}
