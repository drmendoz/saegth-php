...
class Multifuente extends Model {
	...
	function get_Eval($id,$test=null,$eval=null){		
		if(isset($test) && isset($eval)){
			...
			$res = $this->query('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$id.' AND `cod_evaluado`="'.$_SESSION['evaluado']['id'].'" AND `cod_test`="'.$_SESSION['evaluado']['test'].'"',1);
			if($res){
				$otras = $this->query('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$id.' ORDER by fecha DESC');
				if((sizeof($otras))<2){
					$_SESSION['evaluado']['otras']=null;
				...
				$res = $this->query('SELECT `cod_evaluado`, `cod_test`,`resuelto`,`fecha` FROM `multifuente_evaluado` WHERE `id_personal`='.$id.' ORDER by fecha DESC');
				if($res){
									if((sizeof($res))<2){
										$_SESSION['evaluado']['otras']=null;
											$res = $res[0];
