...
function select_id_eval_id_personal($id_eval,$id_personal){
  $sql =  "SELECT * FROM multifuente_relacion WHERE id_evaluado = $id_eval AND id_personal=$id_personal ;";
  $row =  $this->query_($sql,1);
  if($row){
...
function select_all($id){
  return $this->query_('SELECT * FROM multifuente_relacion WHERE `id_evaluado`='. $id .'');
}

function select_all_nivel($id,$niv){
  return $this->query_('SELECT * FROM multifuente_relacion WHERE `id_evaluado`='. $id .' AND `nivel`='.$niv.'');
}
...
function get_min_nivel($id){
  $respuesta =  $this->query_('SELECT MIN(nivel) as nivel FROM multifuente_relacion WHERE `id_evaluado`='. $id .'',1);
  return $respuesta['nivel'];
}
...
function insert(){
  ...
  $sql = "INSERT INTO multifuente_relacion ( id,id_evaluado,id_personal,id_empresa,relacion,nivel,aprovado,tipo_ingreso ) VALUES ( '$this->id','$this->id_evaluado','$this->id_personal','$this->id_empresa','$this->relacion','$this->nivel','$this->aprovado', '$this->tipo_ingreso' )";
