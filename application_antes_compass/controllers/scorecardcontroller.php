...
class ScorecardController extends Controller {
	...
	function generacion($id,$per=""){
		...
		//$obj_sup = $this->Scorecard->query_('SELECT * from scorer_objetivo WHERE id_personal = '.$pid.' AND periodo='.$scorer['periodo'].'');
        $obj_sup = $this->Scorecard->query_('SELECT t.*,ifnull(t.id_padre,t.id)as padre from scorer_objetivo t WHERE id_personal = '.$pid.' AND periodo='.$scorer['periodo'].' order by padre');
		...
		//$obj_def = $this->Scorecard->query_('SELECT * from scorer_objetivo WHERE id_personal = '.$id.' AND periodo='.$scorer['periodo'].'');
        $obj_def = $this->Scorecard->query_('SELECT t.*,ifnull(t.id_padre,t.id)as padre from scorer_objetivo t WHERE id_personal = '.$id.' AND periodo='.$scorer['periodo'].' order by padre');
	...
	function confirmacion($id,$per=""){
		...
		//$obj_def = $this->Scorecard->query('SELECT * from scorer_objetivo WHERE id_personal = '.$id.' AND periodo='.$scorer['periodo'].'');
        $obj_def = $this->Scorecard->query('SELECT t.*,ifnull(t.id_padre,t.id)as padre from scorer_objetivo  t WHERE id_personal = '.$id.' AND periodo='.$scorer['periodo'].' order by padre');
	...
	function vista_scorecard(){
		...
		$this->set('area',$lp->getArea() );
		$this->set('fecha', $scorer_det['periodo']);
		$this->set('id_personal', $_SESSION['USER-AD']['id_personal']);
		...
		$obj_sup = $this->Scorecard->query_('SELECT * FROM scorer_objetivo r WHERE id_personal= ' .$_SESSION['USER-AD']['id_personal']. ' and periodo='.$scorer_det['periodo'].'  and id_padre is null');
			if($obj_sup)
			   {
				$this->set('obj_sup',$obj_sup);
                           }
		...
		$emp = $this->Scorecard->query_('select e.* from empresa e, users u where e.id=u.id_empresa and u.id_personal= '.$_SESSION['USER-AD']['id_personal']);
		$this->set('emp',$emp);
	}
	
	
	function vista_scorecard2(){
		$this->set('title','Alto Desempe&ntilde;o | ScoreCard');
		$this->haySession();
		...
		$this->set('id_empresa', $_SESSION['Empresa']['id']);
                
		$ids = $lp->select_all($_SESSION['Empresa']['id']);
                // var_dump($ids);
		// var_dump($ids);
		// $ids = implode(", ", $ids);
		// $sub_a = $this->Scorecard->query('SELECT * FROM scorer_estado WHERE id_personal IN ('.$ids.') AND activo=1');
		$se = new scorer_estado();
		$sub_a = $se->select_all_by_ids($ids);
		$obj_sup = $this->Scorecard->query_('SELECT * FROM scorer_objetivo r WHERE r.id_padre is null and r.id_padre_sup is null and periodo='.$scorer_det['periodo']);
			if($obj_sup)
			   {
				$this->set('obj_sup__',$obj_sup);
                           }
		
		$this->set('sub_a',$ids);
		
		//$emp = $this->Scorecard->query_("SELECT * FROM scorer_objetivo o WHERE o.id in (1129,1130,1131) and periodo=".$scorer_det['periodo']);
                $emp = $this->Scorecard->query_("SELECT o.id ,o.objetivo, o.indicador,o.inverso,o.unidad, o.lreal, o.lpond, o.meta, o.peso, o.id_padre, o.id_padre_sup  FROM scorer_objetivo o , personal p
                 WHERE o.id_personal=p.id
				     and o.id_padre IS  NULL 
                        AND o.id_padre_sup IS  NULL 
                     AND o.periodo=".$scorer_det['periodo']." 
                    /*AND UPPER(o.objetivo) LIKE UPPER('Plan Estrat%')*/
