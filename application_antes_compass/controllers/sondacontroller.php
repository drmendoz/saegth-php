...
class SondaController extends Controller {
    ...
    */
	...
	function grupos_criticos(){
		...
		$sql =  "SELECT MAX(ID) as id FROM sonda WHERE id_empresa = ".$_SESSION['Empresa']['id'];
	...
	function enviar_recordatorio(){
		...
		if(isset($_POST['button'])){
			foreach ($_POST['id'] as $a => $id) {
				$result = $sonda_user->credencialesSondaTemporal($id,$sonda->getId());
	...
	function getFechas($id_e){
		...
		$sql = 'SELECT id, fecha FROM sonda WHERE id_empresa = '.$id_e.' and fecha != "" order by id desc';
	...
	function get_sondas($id_s){
		...
		$sql = 'SELECT fecha FROM sonda WHERE id_empresa = '.$_SESSION['USER-AD']['id_empresa'].' and id = '.$id_s;
